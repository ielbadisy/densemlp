...
vignette: >
  %\VignetteIndexEntry{Getting Started with mlp}
...
`mlp` trains multilayer perceptrons for tabular data. It accepts a formula or
separate `x` and `y` inputs, preprocesses numeric and categorical predictors,
and returns an `mlp_fit` object with methods for prediction, plotting, metrics,
tuning, and variable importance.
